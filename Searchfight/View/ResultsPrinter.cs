...
namespace SearchFight.View
{
    public class ResultsPrinter
    {
        ...
        public static IEnumerable<string> GetTotalWinner(IEnumerable<SearchResult> searchResults)
        {
            var results = new List<string>();
            ...
            var searchResultsByProgrammingLanguage = searchResults.GroupBy(searchResult => searchResult.ProgrammingLanguage);
            //var searchResultsByProgrammingLanguage2 = searchResultsByProgrammingLanguage.OrderByDescending(searchResult => searchResult.NumberOfResults);
            //searchResultsByProgrammingLanguage.OrderByDescending();
            foreach (var group in searchResultsByProgrammingLanguage)
            {
                var winner = group.OrderByDescending(searchResult => searchResult.NumberOfResults).First();

                results.Add("Total winner: " + winner.ProgrammingLanguage);
            }
